...
O Azure Databricks é uma versão baseada no Microsoft Azure da popular plataforma de Databricks de código aberto.
...
7. Aguarde a conclusão do script. Isso normalmente leva cerca de 5 minutos, mas em alguns casos pode demorar mais. Enquanto espera, revise o artigo [O que é o Azure Databricks?](https://learn.microsoft.com/azure/databricks/introduction/) na documentação do Azure Databricks.
...
1. Baixe o arquivo [**products.csv**](https://raw.githubusercontent.com/MicrosoftLearning/dp-203-azure-data-engineer/master/Allfiles/labs/23/adventureworks/products.csv) para o computador local, salvando-o como **products.csv**. Em seguida, no notebook **Explorar produtos**, no menu **Arquivo**, selecione **Carregar dados para o DBFS**.
1. Na caixa de diálogo **Carregar Dados**, veja o **diretório de destino do DBFS** para onde o arquivo será carregado. Em seguida, selecione a área **Arquivos** e faça o upload do arquivo **products.csv** que você baixou para o seu computador. Quando o upload terminar, selecione **Avançar**
1. No painel **Acessar arquivos de notebooks**, selecione o código PySpark de amostra e copie-o para a área de transferência. Você o usará para carregar os dados do arquivo em um DataFrame. Em seguida, selecione **Concluído**.
...
1. Use a opção de menu **▸ Executar célula** no canto superior direito da célula para executá-la, iniciando e anexando o cluster, se solicitado.
1. Aguarde até que o processo do Spark executado pelo código seja concluído. O código criou um objeto de *dataframe* chamado **df1** a partir dos dados no arquivo carregado.
1. Na célula de código já existente, use o ícone **+** para adicionar uma nova célula de código. Em seguida, insira o código a seguir na nova célula:
...
1. Acima da tabela de resultados, selecione **+** e, em seguida, **Visualização** para exibir o editor de visualização e aplique as seguintes opções:
    - **Tipo de visualização**: barra
    - **Coluna X**: categoria
    - **Coluna Y**: *adicione uma nova coluna e selecione* **ProductID**. *Aplique a* *agregação* **Contagem**.

    Salve a visualização e observe que ela é exibida no notebook assim:
...
Embora muitas análises de dados usem linguagens como Python ou Scala para trabalhar com dados em arquivos, muitas soluções de análise de dados são construídas em bancos de dados relacionais, nos quais os dados são armazenados em tabelas e manipulados usando SQL.
